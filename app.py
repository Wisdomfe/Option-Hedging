...
def mid_price_row(row: pd.Series) -> float:
    ...
    # Fallback per opzioni illiquide: usa lastPrice se disponibile
    return float(last) if pd.notna(last) and last > 0 else np.nan
...
with st.sidebar:
    ...
    st.divider()
    st.subheader("Filtro tradabilità (senza spread)")
    ...
    st.caption("Nota: se bid/ask non sono disponibili, il mid-price usa il lastPrice (se > 0).")
...
def run_analysis():
    ...
    # tradability + mid (SENZA filtro spread)
    def apply_filters(df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Accetta:
        # - bid/ask validi (mid calcolabile) oppure
        # - lastPrice valido (fallback per opzioni illiquide)
        has_mid = (out["bid"] > 0) & (out["ask"] > 0) & (out["ask"] >= out["bid"])
        has_last = out["lastPrice"].notna() & (out["lastPrice"] > 0)
    ...
    if calls_f.empty and puts_f.empty:
        raise ValueError("Dopo i filtri di tradabilità non rimane nessuna opzione. Riduci OI/volume (o mettili a 0).")
    ...
    for K in strike_range:
        ...
        # robust: if one side empty after filters, skip gracefully
    ...
    return {
        ...
        "filters": {"min_oi": min_oi, "min_vol": min_vol},
    }
...
st.caption(
    f"Filtro tradabilità: OI≥{A['filters']['min_oi']}"
    + (f", volume≥{A['filters']['min_vol']}" if A['filters']['min_vol'] and A['filters']['min_vol'] > 0 else "")
    + ". (Se bid/ask mancano, mid=lastPrice se disponibile)"
)
...
# ======================================================
st.subheader("Grafico 1: Differenza (Market − Black-Scholes) usando σ storica (mid-price)")

df_plot = A["df"].copy()

S0 = float(A["S0"])
T = float(A["T"])
sigma_hist = float(A["sigma_hist"])
...
for _, row in df_plot.iterrows():
    ...
    mkt_c = row["Market Call (mid)"]
    mkt_p = row["Market Put (mid)"]

    bs_c = bs_call(S0, K, r, T, sigma_hist)
    bs_p = bs_put(S0, K, r, T, sigma_hist)

    dc = float(mkt_c - bs_c) if pd.notna(mkt_c) and pd.notna(bs_c) else np.nan
    dp = float(mkt_p - bs_p) if pd.notna(mkt_p) and pd.notna(bs_p) else np.nan

    diff_call_hist.append(dc)
    diff_put_hist.append(dp)

x = df_plot["Strike"].astype(float).values
call_y = np.array(diff_call_hist, dtype=float)
put_y  = np.array(diff_put_hist, dtype=float)

fig1 = plt.figure(figsize=(14, 6))
plt.bar(x, call_y, label="Call (Market - BS)", alpha=0.9)
plt.bar(x, put_y,  label="Put (Market - BS)",  alpha=0.9)
plt.axhline(0, linestyle="--", linewidth=1)
plt.axvline(S0, linestyle="--", linewidth=1, label="S0")
...
ST = st.number_input(
    "Prezzo sottostante a scadenza (ST) per valutazione strategia",
    min_value=0.0,
    value=float(A["S0"]),
    step=float(max(1.0, A["S0"] * 0.01)),
    key="strategy_ST"
)
